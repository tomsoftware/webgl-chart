...
export abstract class GpuBufferBase<T extends TypedArray> implements GpuWritableBuffer {
  ...
  /**
   * Returns the index that is nearest by a given value.
   * Expects the underlying buffer to be sorted.
   */
  public findIndex(value: number): number {
    ...
    const pos = ArrayUtilities.interpolationSearch(
      (index) => this.getComponentAt(index, 0),
      minIndex,
      maxIndex,
      value,
    );
    ...
    const lowDelta = Math.abs(value - this.getComponentAt(low, 0));
    const highDelta = Math.abs(value - this.getComponentAt(high, 0));
    return lowDelta <= highDelta ? low : high;
  }

  /** Fill the buffer with values generated by the callback 'calc' */
  public generate(calc: (i: number) => number): this {
    ...
    this.validLength = capacity;
    this.currentDataVersion++;
    return this;
  }

  /**
   * Make sure the buffer is large enough and if possible increase the capacity of 
   *  the buffer */
  ...
  /** Make sure the buffer can add newItemsCount items to the buffer and if 
   *    possible increase the capacity of the buffer */
  public increaseCapacity(newItemsCount: number): this {
    const safeNewItems = Math.max(0, newItemsCount ?? 0);
  ...
  */
  ...
  /** Set the vertexAttribPointer(s) for this buffer */
